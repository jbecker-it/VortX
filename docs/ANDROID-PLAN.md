...
| S03 | Engine bring-up | **fix round 2 in progress** (Sonnet 5 worker; retest 2 via logcat: event path proven healthy but board Load emits ZERO NewState — silent no-op on Rust side; sign-out still not clearing Home; NEW: series detail blank in landscape / visible in portrait; FIXED so far: tablet portrait Home, raw errors, CW progress) | `claude/android-s03-engine-bringup` | merged 2026-07-12. Native path proven: cargo-ndk .so for arm64+x86_64 in both flavors' APKs (inspected), JNI symbols verified, both flavors build + lint clean. Engine process-scoped on `VortXApplication` (Activity-recreation-safe); event bridge non-blocking (tryEmit + DROP_OLDEST; consumers re-pull via getState). Auth via engine ctx Login/Logout; app persists display email only (Keystore-backed), never tokens. Real rail titles from add-on manifests; Coil 3.3.0 (pinned to the Kotlin 2.2 metadata window — do not bump independently). Fixed: 32-bit ABI slices shipped without the engine .so (`abiFilters`). Emulator infeasible in CI container (no KVM) → maintainer on-device test script in the S03 report; run before the next main fast-forward. S04 note: `installed_addons_with_filters` may need adding to `TvosModel` + `get_state_json` (the one near-horizon Rust change); ctx actions dispatch with field=null. |
