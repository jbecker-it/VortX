...
- **A 4K Dolby Vision stream that is still loading is no longer dropped to HDR10 too early (build 165).** The true Dolby Vision lane rebuilds the stream on the fly and a slow debrid source can still be producing data past twenty seconds, but a fixed twenty second start timer silently demoted every such session to the built-in player (HDR10) before Apple's player ever produced a frame. The timer now extends in bounded rounds (up to about a minute total) for as long as the remux is measurably still producing bytes, and demotes on today's schedule the moment it stalls, so a genuinely dead mount still falls back fast. Alongside it, every Dolby Vision and HDR breadcrumb (the display-mode request, a Match Dynamic Range OFF refusal, the remux mount and classify, and the exact demote reason) now also writes to the log users actually export, so any remaining Dolby-Vision-plays-as-HDR case can be pinned from one exported log. Apple TV only.
- **Turkish, Dutch, Polish, and Swedish added to the default Audio and Subtitle language lists in Settings.** The curated default-track language list carried twelve languages and Turkish was not one of them, so a Turkish viewer could not set it as a preferred audio or subtitle language. The shared list now includes them, so the choice appears on iPhone, iPad, Mac, and Apple TV at once.
