...
A hotfix over 0.3.9 that restores instant playback (a source now starts in a second or two instead of stalling for up to half a minute and showing "source failed"), makes trailers actually play, plays dual-layer Dolby Vision as true DV, clears finished titles out of Continue Watching, restores the community source list, sharpens the Streaming Services logos, and hardens account sync. In-place update, nothing resets.
...
- **Add-on trailers play, in your language.** Trailers from add-ons like Strealier are now treated as trailers end to end: they resolve straight from your device in your preferred language and a dead trailer shows "Trailer unavailable" instead of quietly starting the movie. Movies and series, on iPhone, iPad, Mac, and Apple TV.
- **Streaming Services logos are legible for every provider.** Regional and long-tail services now sit on the same clean light plate as the major brands instead of a dark, near-invisible mark, and a service whose logo has not loaded yet shows a legible initial rather than a blank box, on every platform.
