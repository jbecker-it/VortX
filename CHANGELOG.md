...
The Dolby Vision and reliability release. It rolls up the whole 0.3.11 test line (builds 155 to 166). Headline: Dolby Vision from an MKV now plays as true Dolby Vision through Apple's player, delivered as local HLS the way Infuse and Plex do it, after the root cause turned out to be the delivery format rather than any header repair. Alongside it: the Apple TV playback stutter is gone, a slow Dolby Vision source now falls back fast instead of making you wait, audio and subtitle languages gained a fallback chain, and the reliability, security, and add-on work from the 0.3.11 betas. In-place update, nothing resets.
